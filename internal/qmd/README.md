...
**Real vs mock:** When the `qmd` CLI is available, the adapter uses it for real search and index updates. When it is not found, the adapter **FAILS CLOSED** by default (requires explicit `FallbackToMock: true` in config). This ensures production deployments require real QMD. See **docs/01-ARCHITECTURE/BLOCK5_QMD_POPULATION.md** (“Real vs mock”) for how to run with real QMD.
