# Intelligence Mining Design

## Overview

The Intelligence Mining system analyzes proof-of-work artifacts to extract patterns, statistics, and recommendations that improve future task execution. It serves as a foundational component for learning and optimization in Zen-Brain.
...
- Compatibility filtering ensures only workType/workDomain-matching templates are recommended
...
- **Failure-aware**: Recent failures (≥3) downgrade confidence

### Selection reasoning (agent-facing)

The recommender produces **reasoning** strings that explain why a template or configuration was chosen. These are persisted in proof-of-work (`selection_reasoning`) and in task metadata so agents and operators can interpret decisions.

**Template recommendation reasoning:**
- **Exact match:** e.g. "Exact-match history: N runs, X% success" (optionally "last seen D days ago", "freshness penalty applied").
- **Work-type fallback:** "Work-type fallback used; no exact domain history for &lt;workType&gt;/&lt;workDomain&gt;" or "Default selected because compatible history is insufficient".
- **Recency-aware:** reasoning may include "last seen D days ago" and freshness penalty text.

**Failure-aware downgrade:** When recent failures are elevated, reasoning is extended with a warning (e.g. "Warning: recent failures are elevated (mode=N), confidence reduced; consider troubleshooting before proceeding") so the agent can surface caution.

**Configuration recommendation reasoning:** e.g. "Based on N duration samples (P95: X, P99: Y), success rate: Z%".

**Further (richer reasoning):** Deeper explanations (e.g. feature importance, alternative templates with pros/cons) can be added in the recommender and exposed via the same metadata fields.
...
**Intelligence is wired into runtime:**
- Vertical-slice initializes pattern store at `<ZEN_BRAIN_RUNTIME_DIR>/patterns` (default `/tmp/zen-brain-factory/patterns`)
- Creates MiningIntegration
- Sets Factory recommender via `SetRecommender(mining.GetFactoryRecommender())`

**Factory consumes recommendations:**
- When a recommender is configured, `createExecutionPlan()` uses `chooseTemplateAndConfig()`
- Calls `RecommendTemplateWithMetadata` and `RecommendConfiguration`
- Selected template identity, timeout/retry overrides, and metadata (source, confidence, reasoning) are persisted on the task spec and in proof-of-work

**Proof-of-work records actual template:**
- Artifacts include `template_used`, `selection_source`, `selection_confidence`, `selection_reasoning`
- `model_used` is kept for backward compatibility

**Mining after execution:**
- After Factory execution completes, vertical-slice invokes `MineProofOfWorks(ctx)`
- The loop learns from each run (warning only on failure)

**Template tracking:**
- Proof-of-work artifacts include `WorkType` and `WorkDomain`
- Miner prefers `template_used` when present, with `model_used` fallback for older artifacts
...
- [Proof of Work](PROOF_OF_WORK.md) - Proof-of-work artifacts that are mined
- [ZenContext](ZEN_CONTEXT.md) - Tiered memory system for pattern storage
- [Zen Journal](ZEN_JOURNAL.md) - Journal entries used for ReMe protocol
- [PROGRESS.md](../01-ARCHITECTURE/PROGRESS.md) - Project progress tracking
