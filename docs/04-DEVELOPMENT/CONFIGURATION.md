...
When `ZEN_BRAIN_MESSAGE_BUS=redis` is set, the bus is enabled and uses `REDIS_URL` (**required** – no default `redis://localhost:6379`). Session lifecycle events (session.created, session.transitioned, etc.) are published to the configured stream when `session.Config.EventBus` is set.
...
**LLM local worker (Ollama — L0 FALLBACK ONLY):**

> **NOTE:** The primary inference runtime is **llama.cpp** (L1/L2 lanes). Ollama (L0) is a certified fallback path only, used when L1/L2 are unavailable. See `docs/03-DESIGN/LOCAL_LLM_ESCALATION_LADDER.md` and `config/policy/README.md` for the lane hierarchy. **`OLLAMA_BASE_URL` is required** (e.g. `http://localhost:11434` or in-cluster `http://ollama:11434`). The **local-worker** lane uses the real Ollama provider (`internal/llm/ollama_provider.go`) and calls Ollama’s `/api/chat` endpoint. Model and timeout come from gateway config (`local_worker_model`, `local_worker_timeout`). If `OLLAMA_BASE_URL` is unset, the gateway **FAILS CLOSED** (no simulated local worker fallback). **In-cluster Ollama (Block 5):** Deployment is **Helm/Helmfile-based and declarative**. Set `deploy.use_ollama: true` and `deploy.ollama.models: ["qwen3.5:0.8b"]` (or other models) in `config/clusters.yaml`. Run `make dev-up` or `python3 scripts/zen.py env redeploy --env <env>`; values are generated from clusters.yaml and Helmfile deploys the Ollama chart with a **model preload Job** (no manual `kubectl exec ... ollama pull` in the standard path). For emergency or one-off model pulls, use the StatefulSet pod: `kubectl exec -it ollama-0 -n zen-brain -- ollama pull <model>` (or `statefulset/ollama`). See `deploy/README.md` and `charts/zen-brain-ollama/`.

**Mock/degraded paths (Block 3/5):** By default, capabilities are **FAIL CLOSED**. **QMD** requires explicit `FallbackToMock: true` to fall back to mock when the `qmd` CLI is missing or unavailable. **Ledger** requires explicit `ZEN_BRAIN_ALLOW_STUBS=1` to fall back to stub when no DSN is set. The runtime report shows `ledger.mode: stub` when stub is used. To require a capability and fail startup instead of using a fallback, set the corresponding `ZEN_BRAIN_REQUIRE_*` env or `required: true` in config.
