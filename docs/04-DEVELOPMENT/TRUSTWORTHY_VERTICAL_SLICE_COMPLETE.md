...
### Factory Execution
- **Current:** ✅ Wired in vertical slice; bounded execution in isolated workspaces; proof-of-work generated by Factory.
- **Intelligence:** When pattern store is available, Factory uses recommender for template/config selection; proof-of-work includes `template_used` and selection metadata. Mining runs after execution (warning only on failure).

### Proof-of-Work Generation
- **Current:** ✅ Generated by Factory; JSON includes `template_used`, `selection_source`, `selection_confidence`, `selection_reasoning`; backward-compatible `model_used` retained.
