...
func main() {
	...
	// Phase 2: Prepare leader election options
	var leaderElectionOptions []ctrl.Option
	if enableLeaderElection {
		...
		opts, err := leader.PrepareManagerOptionsWithConfig(
			ctx,
			"zen-brain-controller",
			leaderElectionNamespace,
			leConfig,
		)
		if err != nil {
			setupLog.Error(err, "Failed to prepare leader election options")
			os.Exit(1)
		}
	...
	// Phase 3: Build controller manager options
	mgrOpts := ctrl.Options{
		Scheme:                 scheme,
		Metrics:                metricsserver.Options{BindAddress: metricsAddr},
		HealthProbeBindAddress: probeAddr,
		LeaderElection:         false, // Already configured via PrepareManagerOptions
	}
	...
	// Apply leader election options if enabled
	if enableLeaderElection {
		// Note: PrepareManagerOptions returns options that include LeaderElection: true
		// We need to recreate the manager with those options
		mgr, err = ctrl.NewManager(ctrl.GetConfigOrDie(), append([]ctrl.Option{}, leaderElectionOptions...)...)
		if err != nil {
			setupLog.Error(err, "Unable to start manager with leader election")
			os.Exit(1)
		}
	}
